...
class DRCT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.ms_conv_last, self.last_convs = nn.ModuleList(), nn.ModuleList()
            for i in range(len(self.upsample.layers) // 2 - 1):
                self.ms_conv_last.append(nn.Conv2d(num_feat, num_out_ch, 3, 1, 1))
            # self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
            for _ in range(4):
                self.last_convs.append(nn.Conv2d(num_feat, num_out_ch, 3, 1, 1))
            self.adaptive_weight = nn.Linear(1, 4)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x_fuse = self.upsample(x)
            for i in range(len(x_fuse)):
                if i < 2:
                    x_fuse[i] = self.ms_conv_last[i](x_fuse[i])
                    x_fuse[i] = x_fuse[i] / self.img_range + self.mean
                    outs.append(x_fuse[i])
                else:
                    conv_outputs = [conv(x_fuse[i]) for conv in self.last_convs]
                    stacked_outputs = torch.stack(conv_outputs, dim=1)
                    B = x_fuse[i].size(0)
                    weights = self.adaptive_weight(torch.ones(B, 1).to(x_fuse[i].device))
                    weights = F.softmax(weights, dim=1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                    weighted_outputs = (stacked_outputs * weights).sum(dim=1)

                    x_fuse[i] = weighted_outputs / self.img_range + self.mean
                    outs.append(x_fuse[i])
