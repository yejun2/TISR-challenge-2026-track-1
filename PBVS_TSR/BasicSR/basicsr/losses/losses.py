...
def calculate_radiation_energy(inv_distance: torch.Tensor, thermal: torch.Tensor, use_sigma: bool = True):
    """
    inv_distance: (N,1,H,W)  (1 / distance) 형태 권장
    thermal     : (N,3,H,W)  [0,1] range 권장 (또는 정규화된 값)

    return: (N,3,H,W) in [0,1] (min-max norm)
    """
    # thermal 안정화
    eps_T = 1e-6
    thermal = thermal + eps_T
    
    # Stefan–Boltzmann constant
    sigma = 5.670374419e-8 if use_sigma else 1.0

    # thermal^4
    E_total = thermal ** 4  # (N,3,H,W)
    E_r = E_total * (inv_distance ** 2) * sigma  # broadcast (N,3,H,W)

    # min-max normalize per-sample (안정적)
    N = E_r.size(0)
    E_r_flat = E_r.view(N, -1)
    E_min = E_r_flat.min(dim=1)[0].view(N, 1, 1, 1)
    E_max = E_r_flat.max(dim=1)[0].view(N, 1, 1, 1)
    E_r_norm = (E_r - E_min) / (E_max - E_min + 1e-10)
    return E_r_norm

## 추후 수정.
@LOSS_REGISTRY.register()
class SSIMStructureOnlyLoss(nn.Module):
    """
    너가 준 SSIM_StructureOnly를 "loss"로 쓰기 좋게 래핑.
    기본 출력은 (1 - structure_similarity)로 반환.
    """
    ...
    def forward(self, x, y):
        ...
        ssim_n = sigma_xy + self.C3
        ssim_d = torch.sqrt(sigma_x * sigma_y) + self.C3
        struct = (ssim_n / ssim_d).mean()
